...
| 6 | **Shared host MariaDB** 🚧 SECURITY CORE BUILT + PROVEN (branch `control-panel-shared-mariadb`) | shared-db container + provision/lifecycle ops live | **Done:** `vibe-wp-shared-db` compose (no port), `db-provision`/`db-deprovision` (least-priv `vibe_<slug>` DB+user, subnet host-grant, off-`ps` root cred), `shared-db` wrapper subcommand (+rotate-root), backup+weekly timer, root rotation, SF-2 `local_infile=0`, panel admin lifecycle (status/init/rotate). **Adversarial security review GATE: PASS; VPS isolation 31/0 — cross-tenant denial proven from the network, survives root rotation; no-pw-in-ps; backup+rotation live.** **Remaining:** site-provisioning onto shared DB (createSharedDb + external-DB/internal-Redis compose topology, Task 6) + wizard UI (Task 7) + migration (Task 8) + 2-site VPS round. | L | **highest** (cross-tenant leak) |
