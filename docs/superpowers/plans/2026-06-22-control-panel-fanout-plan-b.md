...
- **Exec layer is the only host-spawn site** — every op is an allowlisted argv array with a timeout + redaction. No host spawns from route handlers/UI.
...
**Interfaces:**
- Produces: `VIBE_OPS` gains `up`/`down`/`restart`/`cacheFlush`/`restore`/`backupVerify`/`refresh`/`promote`/`harden`/`wpUpdate`/`wpList`; `buildVibeArgv(siteDir, env, op, extraArgs?)`; `streamVibe(siteDir, env, op, { timeoutMs?, args? })`; `runVibe(siteDir, env, op, { timeoutMs?, args? })`.
...
**Interfaces:**
- Produces: `updatesAvailable({ siteId }) → { plugins: number; core: boolean }` (via `wp` native `--format=json`); `updatesApply({ siteId, what })` (operator) `→ { jobId }`.

- [ ] **Step 1: TDD `parseWpUpdates`** in `parse.ts` (wp-cli `plugin list --update=available --format=json` → count; `core check-update --format=json` → bool). Test with a fixture JSON array, then implement:
...
export const updatesRouter = {
	updatesAvailable: protectedProcedure
		...
		.handler(async ({ input }): Promise<{ plugins: number }> => {
			...
			const out = await runVibe(site.installDir, "prod", "wpList", {
				args: ["plugin", "list", "--update=available", "--format=json"],
			});
