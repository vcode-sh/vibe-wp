...
const md5 = (p: string) =>
	createHash("md5")
		.update(readFileSync(resolve(repoRoot, p)))
		.digest("hex");
...
describe("mu-plugin mirror invariant", () => {
	...
	it("vibe-wp-environment.php is byte-identical in both locations", () => {
		expect(md5("content/mu-plugins/vibe-wp-environment.php")).toBe(
