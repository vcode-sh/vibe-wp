...
describe("mergeSmtpConfig", () => {
	it("site overrides global field-by-field", () => {
		const m = mergeSmtpConfig(
			row({ host: "g", mode: "relay" }),
			row({ host: "s" })
		);
...
describe("toEnv", () => {
	...
	it("omits SMTP_PASSWORD when null (preserve-existing semantics)", () =>
		expect(
			"SMTP_PASSWORD" in toEnv(mergeSmtpConfig(row({ mode: "relay" }), null))
		).toBe(false));
});
...
describe("maskSmtpRow", () => {
	...
	it("hasPassword false when empty/null", () => {
		const m = maskSmtpRow(row());
		expect(m).not.toBeNull();
		expect((m as Record<string, unknown>).hasPassword).toBe(false);
	});
